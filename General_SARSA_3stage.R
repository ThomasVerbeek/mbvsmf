...
s_sa[1,1,3] <- s_sa[1,2,4] <- s_sa[2,3,5] <- s_sa[2,4,6] <- 1.0
s_sa[3,5,7] <- s_sa[3,6,8] <- s_sa[4,7,9] <- s_sa[4,8,10] <- 1.0
s_sa[5,9,7] <- s_sa[5,10,8] <- s_sa[6,11,9] <- s_sa[6,12,10] <- 1.0
# reward_function 
reward_function <- function(previous_state,previous_action,current_state,current_trial,rewards=rew) {
  reward <- 0
  if(current_state == 7) reward <- rewards[current_trial]
  if(current_state == 8) reward <- 10 - rewards[current_trial]
  if(current_state == 9) reward <- rewards[current_trial] - 2
  if(current_state == 10) reward <- 10 - rewards[current_trial] - 2 
  return(reward)
}
# allowed actions
a_s <- rbind(
...
lines(Q_storage[3,6,])
points(rew,col="red")
lines(Q_storage[3,5,],col="red")
points(rew-2, col="blue")
lines(Q_storage[4,7,],col="blue")
points(8 - rew, col="green")
lines(Q_storage[4,8,],col="green")
lines(Q_storage[5,10,],lty=2)
lines(Q_storage[5,9,],col="red",lty=2)
lines(Q_storage[6,11,],col="blue",lty=2)
lines(Q_storage[6,12,],col="green",lty=2)

plot(Q_storage[1,1,],type="l")
lines(Q_storage[1,2,],col="red")
lines(Q_storage[2,3,],lty=2)
lines(Q_storage[2,4,],col="red",lty=2)
